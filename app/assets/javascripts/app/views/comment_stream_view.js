...
app.views.CommentStream = app.views.Base.extend({
  ...
  discardComment: function() {
    ...
    // remove photos
    this.photozoneEl.find("li.publisher-photo").each(_.bind(function(index, element) {
      this.removePhoto($(element));
    }, this));

    // no more photos left...
    this.textareaWrapperEl.removeClass("with-attachments");

    // remove error message
    this.textareaWrapperEl.find(".upload-error").remove();
  ...
  uploadCompleteHandler: function(_id, fileName, response) {
    if (response.success) {
      ...
      this.textareaWrapperEl.find(".photodropzone-container").first().after(
  ...
  _addPhotoPlaceholder: function() {
    var id = this.photozoneEl.find("li.publisher-photo").length;

    this.photozoneEl.append(
      "<li class=\"publisher-photo loading\" style=\"position:relative;\">" +
      "  <div class=\"progress\">" +
    ...
    this.textareaWrapperEl.addClass("with-attachments");
  },
  ...
  _removePhoto: function(evt) {
    var photo = $(evt.target).parents(".publisher-photo");
    ...
    $.ajax({
      ...
      success: function() {
        $.when(photo.fadeOut(400)).then(function() {
          ...
          if (self.$(".publisher-photo").length === 0) {
            // no more photos left...
            self.textareaWrapperEl.removeClass("with-attachments");
          }
