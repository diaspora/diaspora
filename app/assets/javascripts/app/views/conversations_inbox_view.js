...
app.views.ConversationsInbox = Backbone.View.extend({
  ...
  renderConversation: function(conversationId) {
    if (parseInt("" + conversationId, 10)) { // Ensures that conversationId is a readable number
      var self = this;
      $.ajax({
  ...
  selectConversation: function(conversationId) {
    this.$el.find("#conversation-inbox .stream_element").removeClass("selected");
    if (conversationId) {
      this.$el.find("#conversation-inbox .stream_element[data-guid='" + conversationId + "']").addClass("selected");
    }
  },
