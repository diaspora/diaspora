...
class AspectMembershipsController < ApplicationController
  ...
  def destroy
    ...
    fail ActiveRecord::RecordNotFound unless aspect.present? && contact.present?

    fail Diaspora::NotMine unless current_user.mine?(aspect) &&
                                  current_user.mine?(contact)

    membership = contact.aspect_memberships.where(aspect_id: aspect.id).first

    fail ActiveRecord::RecordNotFound unless membership.present?
