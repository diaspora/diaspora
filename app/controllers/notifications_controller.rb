...
class NotificationsController < ApplicationController
  ...
  def index
    ...
    if params[:type] && Notification.types.key?(params[:type])
    ...
    Notification.types.each_with_object(current_user.unread_notifications.group_by(&:type)) do|(name, type), notifications|
      @grouped_unread_notification_counts[name] = notifications.key?(type) ? notifications[type].count : 0
    end
