...
class Contact < ActiveRecord::Base
  ...
  def dispatch_request
    ...
    Postzord::Dispatcher.build(user, request).post
    request
  end

  def generate_request
    Request.diaspora_initialize(from: user.person,
                                to: person,
                                into: aspects.first)
  end

  def receive_shareable(shareable)
    ShareVisibility.create!(shareable_id: shareable.id, shareable_type: shareable.class.base_class.to_s, contact_id: id)
  end
  ...
  def contacts
    ...
    incoming_aspects = Aspect.where(
      ...
      contacts_visible: true).joins(:contacts).where(
        contacts: { person_id: user.person_id }).select('aspects.id')
    incoming_aspect_ids = incoming_aspects.map(&:id)
    similar_contacts = Person.joins(contacts: :aspect_memberships).where(
      aspect_memberships: { aspect_id: incoming_aspect_ids }).where(people[:id].not_eq(user.person.id)).select('DISTINCT people.*')
  end
