...
class Invitation < ActiveRecord::Base
  ...
  def send!
    if email_like_identifer
      EmailInviter.new(identifier, sender).send!
    else
  ...
  def recipient_not_on_pod?
    ...
    if recipient.username?
      errors[:recipient] << "The user '#{identifier}' (#{recipient.diaspora_handle}) is already on this pod, so we sent them a share request"
    end
  end
  ...
  def valid_identifier?
    ...
    if self.service == 'email'
      unless identifier.match(Devise.email_regexp)
