...
class Person < ActiveRecord::Base
  ...
  def initialize(params = {})
    profile_set = params.key?(:profile) || params.key?('profile')
    params[:profile_attributes] = params.delete(:profile) if params.key?(:profile) && params[:profile].is_a?(Hash)
    super
  ...
  def self.find_from_guid_or_username(params)
    ...
    fail ActiveRecord::RecordNotFound unless p.present?
    p
  end
  ...
  def self.search(query, user)
    return where('1 = 0') if query.to_s.blank? || query.to_s.length < 2

    sql, tokens = search_query_string(query)
  ...
  def public_key_hash
    Base64.encode64(OpenSSL::Digest::SHA256.new(exported_key).to_s)
  end
  ...
  def exported_key=(new_key)
    fail "Don't change a key" if serialized_public_key
  ...
  def self.local_by_account_identifier(identifier)
    person = by_account_identifier(identifier)
  ...
  def shares_with(user)
    user.contacts.receiving.where(person_id: id).first if user
  end
  ...
  def update_url(url)
    ...
    newuri += ":#{location.port}" unless %w(80 443).include?(location.port.to_s)
    newuri += '/'
    update_attributes(url: newuri)
  end
  ...
  def fix_profile
    Webfinger.new(diaspora_handle).fetch
    reload
  end
end
