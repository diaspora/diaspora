class Services::Facebook < Service
  ...
  def post(post, _url = '')
    Rails.logger.debug("event=post_to_service type=facebook sender_id=#{user_id}")
  ...
  def profile_photo_url
    "https://graph.facebook.com/#{uid}/picture?type=large&access_token=#{URI.escape(access_token)}"
  end

  def delete_post(post)
    if post.present? && post.facebook_id.present?
      Rails.logger.debug("event=delete_from_service type=facebook sender_id=#{user_id}")
      delete_from_facebook("https://graph.facebook.com/#{post.facebook_id}/", access_token: access_token)
    end
  end
