...
class User < ActiveRecord::Base
  ...
  def ugly_accept_invitation_code
    invitations_to_me.first.sender.invitation_code
  rescue Exception => e
    nil
  end
  ...
  def is_shareable_hidden?(shareable)
    ...
    if hidden_shareables.key?(shareable_type)
  ...
  def toggle_hidden_shareable(share)
    ...
    if hidden_shareables.key?(key) && hidden_shareables[key].include?(share_id)
  ...
  def update_user_preferences(pref_hash)
    ...
    pref_hash.keys.each do |key|
      if pref_hash[key] == 'true'
        ...
        block = user_preferences.where(email_type: key).first
        if block
          block.destroy
        end
      end
  ...
  def disable_getting_started
    update_attribute(:getting_started, false) if self.getting_started?
  end
  ...
  def dispatch_post(post, opts = {})
    FEDERATION_LOGGER.info("user:#{id} dispatching #{post.class}:#{post.guid}")
  ...
  def notify_if_mentioned(post)
    return unless contact_for(post.author) && post.respond_to?(:mentions?)

    post.notify_person(person) if post.mentions? person
  end
  ...
  def like_for(target)
    if target.likes.loaded?
      return target.likes.detect { |like| like.author_id == person.id }
    else
      return Like.where(author_id: person.id, target_type: target.class.base_class.to_s, target_id: target.id).first
    end
  end
  ...
  def seed_aspects
    ...
    if AppConfig.settings.autofollow_on_join?
      ...
      share_with(default_account, aq) if default_account
    end
    aq
  end
  ...
  def clearable_fields
    attributes.keys - %w(id username encrypted_password created_at updated_at locked_at serialized_private_key getting_started disable_mail show_community_spotlight_in_stream email remove_after)
  end
end
