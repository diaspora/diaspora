...
module User::Querying
  ...
  def visible_ids_from_sql(klass, opts = {})
    ...
    post_ids += klass.connection.select_values("#{construct_public_followings_sql(opts).to_sql} LIMIT #{opts[:limit]}").map(&:to_i)
  end
  ...
  def construct_shareable_from_others_query(opts)
    conditions = {
      ...
      contacts: { user_id: id, receiving: true }
    }

    conditions[:type] = opts[:type] if opts.key?(:type)
  ...
  def block_for(person)
    return nil unless person
    blocks.where(person_id: person.id).first
  end
  ...
  def aspects_with_shareable(base_class_name_or_class, shareable_id)
    ...
    aspects.joins(:aspect_visibilities).where(aspect_visibilities: { shareable_id: shareable_id, shareable_type: base_class_name })
  end

  def contact_for_person_id(person_id)
    Contact.where(user_id: id, person_id: person_id).includes(person: :profile).first
  end
