...
module Workers
  class RemoveOldUser < Base
    ...
    def safe_remove_after
      ...
      Time.now -         (AppConfig.settings.maintenance.remove_old_users.after_days.to_i).days -         (AppConfig.settings.maintenance.remove_old_users.warn_days.to_i).days
    end
    ...
    def perform(user_id)
      ...
      if AppConfig.settings.maintenance.remove_old_users.enable?
        ...
        if user.remove_after < Time.now && user.last_seen < safe_remove_after
          user.close_account!
        end
      end
    end
