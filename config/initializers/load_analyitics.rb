...
if Rails.env == 'production'
  Diaspora::Application.configure do
    if AppConfig.privacy.google_analytics_key.present?
      ...
      config.middleware.use Rack::GoogleAnalytics, :tracker => AppConfig.privacy.google_analytics_key
    end

    if AppConfig.privacy.piwik.enable?
      require 'rack/piwik'
      config.gem 'rack-piwik', :lib => 'rack/piwik'
      config.middleware.use Rack::Piwik, :piwik_url => AppConfig.privacy.piwik.host, 
                                         :piwik_id => AppConfig.privacy.piwik.site_id
    end
  end
end
