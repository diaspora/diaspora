module Diaspora
  module Encryptable
    ...
    def verify_signature(signature, person)
      if person.nil?
        Rails.logger.info("event=verify_signature status=abort reason=no_person guid=#{guid}")
        return false
      elsif person.public_key.nil?
        Rails.logger.info("event=verify_signature status=abort reason=no_key guid=#{guid}")
        return false
      elsif signature.nil?
        Rails.logger.info("event=verify_signature status=abort reason=no_signature guid=#{guid}")
        return false
      end
      log_string = "event=verify_signature status=complete guid=#{guid}"
    ...
    def sign_with_key(key)
      ...
      log_hash.merge(model_id: id) if self.respond_to?(:persisted?)
    ...
    def signable_accessors
      ...
      %w(author_signature parent_author_signature).each do |acc|
    ...
    def signable_string
      signable_accessors.collect do |accessor|
        (send accessor.to_sym).to_s
      end.join(';')
    end
  end
end
