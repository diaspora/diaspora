...
class RelayableRetraction < SignedRetraction
  ...
  def perform(_receiving_user)
    ...
    if !parent_author_signature.nil? || parent.author.remote?
      # Don't destroy a relayable unless the top-level owner has received it, otherwise it may not get relayed
      target.destroy
      Rails.logger.info("event=relayable_retraction status =complete target_type=#{target_type} guid =#{target_guid}")
    end
  end
  ...
  def receive(recipient, sender)
    if target.nil?
      ...
      return
    elsif parent.author == recipient.person && self.target_author_signature_valid?
      # this is a retraction from the downstream object creator, and the recipient is the upstream owner
      self.parent_author_signature = sign_with_key(recipient.encryption_key)
      ...
      Rails.logger.info("event=receive status=abort reason='object signature not valid' recipient=#{recipient.diaspora_handle} sender=#{parent.author.diaspora_handle} payload_type=#{self.class} parent_id=#{parent.id}")
      return
    end
    self
  end
