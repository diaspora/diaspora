    ...
    module Shareable
      ...
      module InstanceMethods
        ...
        def receive(user, person)
          ...
          self.class.transaction do
            ...
            if local_shareable && verify_persisted_shareable(local_shareable)
              receive_persisted(user, person, local_shareable)

            elsif !local_shareable
              receive_non_persisted(user, person)

            else
              Rails.logger.info("event=receive payload_type=#{self.class} update=true status=abort sender=#{diaspora_handle} reason='update not from shareable owner' existing_shareable=#{id}")
              false
            end
          end
        end
        ...
        def verify_persisted_shareable(persisted_shareable)
          persisted_shareable.author_id == author_id
        end
        ...
        def receive_persisted(user, person, local_shareable)
          ...
          if known_shareable
            if known_shareable.mutable?
              known_shareable.update_attributes(attributes.except('id'))
              true
            else
              Rails.logger.info("event=receive payload_type=#{self.class} update=true status=abort sender=#{diaspora_handle} reason=immutable") # existing_shareable=#{known_shareable.id}")
              false
            end
          else
            user.contact_for(person).receive_shareable(local_shareable)
            user.notify_if_mentioned(local_shareable)
            Rails.logger.info("event=receive payload_type=#{self.class} update=true status=complete sender=#{diaspora_handle}") # existing_shareable=#{local_shareable.id}")
            true
          end
        end
        ...
        def receive_non_persisted(user, person)
          if save
            ...
            Rails.logger.info("event=receive payload_type=#{self.class} update=false status=complete sender=#{diaspora_handle}")
            true
          else
            Rails.logger.info("event=receive payload_type=#{self.class} update=false status=abort sender=#{diaspora_handle} reason=#{errors.full_messages}")
            false
          end
        end
      end
