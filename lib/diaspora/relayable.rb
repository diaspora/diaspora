...
module Diaspora
  module Relayable
    ...
    def author_is_not_ignored
      if self.new_record? && parent.present?
        post_author = parent.author
        relayable_author = author

        if post_author.local? && post_author.owner.ignored_people.include?(relayable_author)
          errors.add(:author_id, 'This person is ignored by the post author')
    ...
    def receive(user, person = nil)
      comment_or_like = self.class.where(guid: guid).first || self
      ...
      unless comment_or_like.parent_author == user.person || comment_or_like.verify_parent_author_signature
        Rails.logger.info("event=receive status=abort reason='object signature not valid' recipient=#{user.diaspora_handle} sender=#{parent.author.diaspora_handle} payload_type=#{self.class} parent_id=#{parent.id}")
        return
      end
    ...
    # @return [Object]
    def after_receive(_user, _person)
      self
    end

    def initialize_signatures
      # sign relayable as model creator
      self.author_signature = sign_with_key(author.owner.encryption_key)

      if !parent.blank? && author.owns?(parent)
    ...
    def verify_parent_author_signature
      verify_signature(parent_author_signature, parent.author)
    end

    # @return [Boolean]
    def signature_valid?
      verify_signature(author_signature, author)
    end

    # @abstract
    # @return [Class]
    def parent_class
      fail NotImplementedError.new('you must override parent_class in order to enable relayable on this model')
    end

    # @abstract
    # @return An instance of Relayable#parent_class
    def parent
      fail NotImplementedError.new('you must override parent in order to enable relayable on this model')
    end
