  ...
  class MagicSigEnvelope
    ...
    def self.parse(doc)
      ...
      if env.encoding != 'base64url'
        fail ArgumentError, "Magic Signature data must be encoded with base64url, was #{env.encoding}"
      end
      ...
      unless 'RSA-SHA256' == env.alg
        fail ArgumentError, "Magic Signature data must be signed with RSA-SHA256, was #{env.alg}"
      end
