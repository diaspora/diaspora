describe("app.views.ConversationsInbox", function() {
  describe("initialize", function() {
    it("call app.views.ConversationsForm()", function() {
  ...
  describe("renderConversation", function() {
    beforeEach(function() {
      ...
      var conversations = $("#conversation-inbox .stream_element");
    ...
    it("doesn't do anything if no conversation id is passed", function() {
      spyOn($, "ajax");
      this.target.renderConversation();
      expect($.ajax).not.toHaveBeenCalled();
      expect($(".stream_element.selected").length).toBe(0);
    });

    it("doesn't do anything if conversationId is not a readable number", function() {
      spyOn($, "ajax");
      this.target.renderConversation("yolo");
      expect($.ajax).not.toHaveBeenCalled();
      expect($(".stream_element.selected").length).toBe(0);
    });
  ...
  describe("selectConversation", function() {
    beforeEach(function() {
      spec.loadFixture("conversations_read");
      this.conversationId = $("#conversation-inbox .stream_element").first().data("guid");
      this.target = new app.views.ConversationsInbox();
      $("#conversation-inbox .stream_element").addClass("selected");
    });

    it("unselect every conversation if called with no parameters", function() {
      expect($("#conversation-inbox .stream_element.selected").length).not.toBe(0);
      this.target.selectConversation();
      expect($("#conversation-inbox .stream_element.selected").length).toBe(0);
    });

    it("selects the given conversation", function() {
      expect($("#conversation-inbox .stream_element.selected").length).not.toBe(1);
      this.target.selectConversation(this.conversationId);
      expect($("#conversation-inbox .stream_element.selected").length).toBe(1);
      expect($("#conversation-inbox .stream_element.selected").data("guid")).toBe(this.conversationId);
    });
  });
  ...
  describe("displayConversation", function() {
    ...
    it("call app.router.navigate with correct parameters", function() {
