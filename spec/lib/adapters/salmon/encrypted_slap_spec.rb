...
describe Adapters::Salmon::EncryptedSlap do
  before do
    @post = alice.post(:status_message, :text => "hi", :to => alice.aspects.create(:name => "abcd").id)
    @created_salmon = Adapters::Salmon::EncryptedSlap.create_by_user_and_activity(alice, @post.to_diaspora_xml)
  end

  describe '#create' do
    it 'makes the data in the signature encrypted with that key' do
      key_hash = {'key' => @created_salmon.aes_key, 'iv' => @created_salmon.iv}
      decoded_string = Adapters::Salmon::EncryptedSlap.decode64url(@created_salmon.magic_sig.data)
  ...
  describe "#process_header" do
    before do
      @new_slap = Adapters::Salmon::EncryptedSlap.new
  ...
  context 'marshalling' do
    ...
    let(:parsed_salmon) { Adapters::Salmon::EncryptedSlap.from_xml(xml, alice)}
