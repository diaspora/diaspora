...
describe Statistics do
  def result_should_equal(actual)
    ...
    @result.each do |expected_hash|
      expect(actual.find do |actual_hash|
  ...
  %w(posts_count comments_count invites_sent_count tags_followed_count mentions_count sign_in_count contacts_sharing_with_count).each do |method|
    ...
    unless %w(sign_in_count tags_followed_count).include?(method)
