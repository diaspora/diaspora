...
.values
  ...
  describe 'oembed' do
    it 'should queue a GatherOembedData for all valid links it finds' do
      sm = Factory.build(:status_message, :text => 'http://youtube.com is so cool.  so is http://flickr.com and https://joindiaspora.com')
      Resque.should_receive(:enqueue).with(Jobs::GatherOEmbedData, instance_of(Fixnum), instance_of(String)).exactly(2).times
      sm.save
    end
  end
end
