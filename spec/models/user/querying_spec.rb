...
describe User::Querying, type: :model do
  ...
  describe '#visible_shareables' do
    ...
    context 'with many posts' do
      ...
      it 'works' do # The set up takes a looong time, so to save time we do several tests in one
        ...
        expect(bob.visible_shareables(Post).map(&:id)).to eq(bob.visible_shareables(Post, by_members_of: bob.aspects.map(&:id)).map(&:id)) # it is the same when joining through aspects

        # checks the default sort order
        expect(bob.visible_shareables(Post).sort_by(&:created_at).map(&:id)).to eq(bob.visible_shareables(Post).map(&:id).reverse) # it is sorted updated_at desc by default
        ...
        expect(bob.visible_shareables(Post, opts).map(&:id)).to eq(bob.visible_shareables(Post, opts.merge(by_members_of: bob.aspects.map(&:id))).map(&:id))
        expect(bob.visible_shareables(Post, opts).sort_by(&:created_at).map(&:id)).to eq(bob.visible_shareables(Post, opts).map(&:id).reverse)
        ...
        expect(bob.visible_shareables(Post, opts).map(&:id)).to eq(bob.visible_shareables(Post, opts.merge(by_members_of: bob.aspects.map(&:id))).map(&:id))
        expect(bob.visible_shareables(Post, opts).sort_by(&:created_at).map(&:id)).to eq(bob.visible_shareables(Post, opts).map(&:id).reverse)
        expect(bob.visible_shareables(Post, opts).map(&:id)).to eq(bob.visible_shareables(Post, limit: 40)[15...30].map(&:id)) # pagination should return the right posts
  ...
  describe '#posts_from' do
    ...
    it 'displays recent posts first' do
      ...
      expect(alice.posts_from(@user3.person).map(&:id)).to eq([msg4, msg3, @public_message].map(&:id))
    end
  end
end
