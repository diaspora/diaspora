...
shared_examples_for "it is mentions container" do
  ...
  describe ".before_create" do
    it "backports mention syntax to old syntax" do
      ...
      obj = FactoryBot.build(described_class.to_s.underscore.to_sym, text: text, author: alice.person)
      obj.save
    ...
    it "doesn't backport mention syntax if author is not local" do
      text = "mention @{#{people[0].diaspora_handle}} text"
      obj = FactoryBot.build(described_class.to_s.underscore.to_sym, text: text, author: remote_raphael)
      obj.save
