...
describe ExportUserWorker do
  ...
  it 'calls export! on user with given id' do
    ...
    ExportUserWorker.new.perform(alice.id)
  end
  ...
  context "concurrency" do
    ...
    it "schedules a job for later when already another parallel export job is running" do
      expect(Sidekiq::Workers).to receive(:new).and_return(
        [[pid, SecureRandom.hex(4), {"payload" => {"class" => "ExportUserWorker"}}]]
      )

      expect(ExportUserWorker).to receive(:perform_in).with(kind_of(Integer), alice.id)
    ...
    it "runs the export when the own running job" do
      expect(Sidekiq::Workers).to receive(:new).and_return(
        [[pid, Thread.current.object_id.to_s(36), {"payload" => {"class" => "ExportUserWorker"}}]]
      )

      expect(ExportUserWorker).not_to receive(:perform_in).with(kind_of(Integer), alice.id)
      expect(alice).to receive(:perform_export!)
    ...
    it "runs the export when some other job is running" do
      expect(Sidekiq::Workers).to receive(:new).and_return(
        [[pid, SecureRandom.hex(4), {"payload" => {"class" => "OtherJob"}}]]
      )
