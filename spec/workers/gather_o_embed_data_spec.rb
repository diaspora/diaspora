...
describe GatherOEmbedDataWorker do
  before do
    @flickr_oembed_data = {
  ...
  describe '.perform' do
    it 'requests not data from the internet' do
      GatherOEmbedDataWorker.new.perform(@status_message.id, @flickr_photo_url)
    ...
    it 'creates no cache entry for unsupported pages' do
      GatherOEmbedDataWorker.new.perform(@status_message.id, @no_oembed_url)
    ...
    it 'gracefully handles a deleted post' do
      expect {
        GatherOEmbedDataWorker.new.perform(0, @flickr_photo_url)
      }.to_not raise_error
    end
  end
end
