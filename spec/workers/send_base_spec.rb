...
describe SendBaseWorker do
  it "retries first time after at least 256 seconds" do
    retry_delay = SendBaseWorker.new.send(:seconds_to_delay, 1)
  ...
  it "increases the interval for each retry" do
    (2..19).each do |count|
      expect(SendBaseWorker.new.send(:seconds_to_delay, count)).to be >= ((count + 3)**4)
    end
    ...
    expect(SendBaseWorker.new.send(:seconds_to_delay, 9)).to be >= 20_736
    expect(SendBaseWorker.new.send(:seconds_to_delay, 19)).to be >= 234_256
  end
end
